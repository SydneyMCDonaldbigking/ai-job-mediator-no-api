...
@pytest.mark.anyio
async def test_run_manual_seek_search_scrapes_keywords_concurrently(
    monkeypatch: pytest.MonkeyPatch,
):
    active = 0
    max_active = 0
    ...
    async def fake_generate_search_queries(*, resume, language, default_location):
        return GeneratedSearchQueries(
            ...
            keywords=["python engineer", "platform engineer"],
    ...
    async def fake_scrape_seek_search_results(*, keyword: str, location: str):
        ...
        return [
            SeekRawJob(
                ...
                company="Example",
                location=location,
                job_url=f"https://www.seek.com.au/job/{keyword}",
            )
        ]

    monkeypatch.setattr(seek_search_module, "generate_search_queries", fake_generate_search_queries)
    monkeypatch.setattr(seek_search_module, "scrape_seek_search_results", fake_scrape_seek_search_results)
