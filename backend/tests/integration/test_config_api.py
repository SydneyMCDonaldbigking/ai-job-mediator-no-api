...
class TestLlmTest:
    ...
    @patch("app.routers.config.check_llm_health", new_callable=AsyncMock)
    @patch("app.routers.config._load_config")
    async def test_connection_test_uses_health_check_default_prompt(
        self,
        mock_load,
        mock_health,
        client,
    ):
        mock_load.return_value = {"provider": "gemini", "model": "gemini-2.5-flash"}
        mock_health.return_value = {
            "healthy": True,
            "provider": "gemini",
        ...
        assert resp.status_code == 200
        assert mock_health.await_args.kwargs["include_details"] is True
        assert mock_health.await_args.kwargs.get("test_prompt") is None
