...
from app.career_ops.scheduled_scan import start_scheduled_scan_loop, stop_scheduled_scan_loop
...
async def lifespan(app: FastAPI):
    ...
    await start_scheduled_scan_loop()
    yield
    ...
    try:
        await stop_scheduled_scan_loop()
    except Exception as e:
        logger.error(f"Error stopping scheduled scan loop: {e}")
...
app.include_router(scheduled_scan_router, prefix="/api/v1")
app.include_router(career_ops_router, prefix="/api")
