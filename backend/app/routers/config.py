...
from app.career_ops.scanner import load_portals_config, save_portals_config
...
async def test_llm_connection(request: LLMConfigRequest | None = None) -> dict:
    ...
    return await check_llm_health(config, include_details=True)
...
@router.get("/portals", response_model=PortalsConfig)
async def get_portals_config_endpoint() -> PortalsConfig:
    """Get the active portal scanner configuration."""
    return PortalsConfig.model_validate(load_portals_config())


@router.put("/portals", response_model=PortalsConfig)
async def update_portals_config_endpoint(request: PortalsConfig) -> PortalsConfig:
    """Validate and save the portal scanner configuration."""
    saved = save_portals_config(request.model_dump(exclude_none=True))
    if isinstance(saved, dict):
        return PortalsConfig.model_validate(saved)
    return request
