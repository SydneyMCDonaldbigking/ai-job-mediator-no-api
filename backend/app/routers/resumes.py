...
async def _store_or_replace_master_resume(
    markdown_content: str,
    filename: str | None,
) -> dict[str, Any]:
    """Keep a single master resume record and overwrite it on new upload."""
    existing_master = db.get_master_resume()
...
async def _store_language_resume(
    markdown_content: str,
    filename: str | None,
    resume_language: str,
) -> dict[str, Any]:
    if resume_language == "en":
        return await _store_or_replace_master_resume(markdown_content, filename)
...
async def upload_resume(
    ...
    # Keep a single stable master resume id across repeated uploads so
    # thread/session references continue to point at the active master record.
    normalized_language = resume_language.strip().lower()
    if normalized_language not in {"en", "ja", "zh"}:
        raise HTTPException(status_code=400, detail="Unsupported resume language")

    resume = await _store_language_resume(
        markdown_content,
        file.filename,
        normalized_language,
    )
    ...
    db.update_multilingual_resume_asset(normalized_language, resume["resume_id"])
