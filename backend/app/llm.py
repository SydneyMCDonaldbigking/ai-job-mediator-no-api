...
LLM_HEALTH_CHECK_MAX_TOKENS = 32
LLM_HEALTH_CHECK_PROMPT = "Reply with exactly hello"
LLM_HEALTH_CHECK_RETRIES = 2
LLM_HEALTH_CHECK_RETRY_DELAY_SECONDS = 1
...
def _is_transient_health_check_error(message: str) -> bool:
    """Return True when a health check failure is likely temporary."""
...
async def check_llm_health(
    ...
    *,
    ...
    # Use a deterministic prompt that reliably produces visible text across
    # reasoning-capable models like Gemini 2.5 Flash.
    prompt = test_prompt or LLM_HEALTH_CHECK_PROMPT
    ...
    try:
        ...
        for attempt in range(LLM_HEALTH_CHECK_RETRIES + 1):
            try:
                ...
                if (
                    ...
                    raise
                await asyncio.sleep(LLM_HEALTH_CHECK_RETRY_DELAY_SECONDS * (attempt + 1))

        if response is None:
            raise last_error or ValueError("LLM health check did not return a response")
...
def _appears_truncated(data: dict) -> bool:
    ...
    # Empty work history is a stronger truncation signal than optional sections
    # like education, which can legitimately be absent in some resumes.
    suspicious_empty_arrays = ["workExperience", "skills"]
